...
`eval/baselines/probes.approval.json` is a separate, manually reviewed record. Its exact schema binds the canonical-JSON SHA-256 of both the current `eval/baselines/probes.json` and the current `eval/probes/probes.json`, the complete `engineVersion`/`corpusFingerprint`/`layerFingerprint` triple, reviewer, review date, rationale, and prior baseline provenance. Canonical JSON makes those logical-document digests stable across LF and CRLF checkouts.

The gauntlet validates the approval against the baseline, the current probe definitions, and the freshly measured fixture identity. A missing, malformed, stale, or mismatching approval fails G8 and therefore produces `REJECT`; `--update-baseline` writes only a candidate baseline and never writes its approval. Updating a baseline requires a separate review record with new hashes, identity, rationale, and prior provenance.
