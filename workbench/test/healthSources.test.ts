...
describe('health source semantics', () => {
  it('mirrors G3 active fixture coverage, including implicit fixture-id coverage', async () => {
    ...
    // 117 = 58 founding concepts + the 20 round-1 books-harvest packs +
    ...
    expect(coverage).toHaveLength(117);
    expect(coverage.filter((entry) => entry.status === 'active')).toHaveLength(117);
