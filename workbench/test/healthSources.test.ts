...
describe('health source semantics', () => {
  it('mirrors G3 active fixture coverage, including implicit fixture-id coverage', async () => {
    ...
    // 116 = 58 founding concepts + the 20 round-1 books-harvest packs +
    ...
    // (2026-08-21 Phase-4 P4.10a/P4.10b/P4.10c). This mirror moves
    // whenever a concept wave is admitted.
    expect(coverage).toHaveLength(116);
    expect(coverage.filter((entry) => entry.status === 'active')).toHaveLength(116);
